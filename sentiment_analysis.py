...
def read_csv():
    '''读取商品评论数据文件'''
    comment_data = pd.read_csv('goods.csv', encoding='utf-8',
                               sep='\n', index_col=None)
    #返回评论作为参数
    return comment_data
...
def train():
    ...
    sentiment.save('ysld.marshal.3')#python2保存的是sentiment.marshal；python3保存的是sentiment.marshal.3
...
def data_virtualization():
    '''分析结果可视化，以条形图为测试样例'''
    font = FontProperties(fname='songti.ttf', size=14)
...
def main():
     #processed_data('goods')#数据清洗
     #train()  # 训练正负向商品评论数据集

     test('goods', 'ysldresult')
